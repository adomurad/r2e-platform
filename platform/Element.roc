...
Element := {
	...
	## ```
	click! : Element => Try({}, [WebDriverError(Str), ElementNotFound(Str)])
	click! = |element| {
		{ session_id, element_id, locator, selector_text } = element
		...
		Host.element_click!(session_id, element_id).map_err(InternalError.handle_element_error)?
	...
	# ## ```
	# get_text! : Element => Try(Str, [WebDriverError Str, ElementNotFound Str])
	# get_text! = |element|
	...
	# ## ```
	# get_value! : Element => Try(a, [ElementNotFound Str, PropertyTypeError Str, WebDriverError Str]) where a implements Decoding
	# get_value! = |element|
	...
	# ## ```
	# is_selected! : Element => Try([Selected, NotSelected], [WebDriverError Str, ElementNotFound Str])
	# is_selected! = |element|
	...
	#     )
	#
	#     result = Host.element_is_selected!(session_id, element_id) |> Try.map_err(InternalError.handle_element_error)?
	#
	#     if result == "true" {
	#         Ok(Selected)
	#     } else {
	...
	# ## ```
	# is_visible! : Element => Try([Visible, NotVisible], [WebDriverError Str, ElementNotFound Str])
	# is_visible! = |element|
	#     { selector_text } = Internal.unpack_element_data(element)
	#
	#     DebugMode.run_if_verbose!(
	...
	# ## ```
	# get_attribute! : Element, Str => Try(Str, [WebDriverError Str, ElementNotFound Str])
	...
	#     )
	#
	#     Host.element_get_attribute!(session_id, element_id, attribute_name) |> Try.map_err(InternalError.handle_element_error)
	#
	# ## Get **attribute** of an `Element`.
	# ##
	# ## **Attributes** are values you can see in the HTML DOM, like *<input class"test" type="password" />*
	...
	# ## ```
	# get_attribute_or_empty! : Element, Str => Try((Result Str, [Empty])) [WebDriverError Str, ElementNotFound Str]
	...
	#     )
	#
	#     result = Host.element_get_attribute!(session_id, element_id, attribute_name) |> Try.map_err(InternalError.handle_element_error)?
	#
	#     if result == "" {
	#         Ok(Err(Empty))
	#     } else {
	...
	# ## ```
	# get_property! : Internal.Element, Str => Try(a, [ElementNotFound Str, PropertyTypeError Str, WebDriverError Str]) where a implements Decoding
	...
	# ## is_checked = name_input |> Element.get_property!("checked")?
	# ## match is_checked {
	# ##     Ok(value) => value |> Assert.should_be(False)
	...
	# ## ```
	# get_property_or_empty! : Element, Str => Try((Result a, [Empty])) [WebDriverError Str, ElementNotFound Str, PropertyTypeError Str] where a implements Decoding
	...
	#     )
	#
	#     result_str = Host.element_get_property!(session_id, element_id, property_name) |> Try.map_err(InternalError.handle_element_error)?
	#
	#     if result_str == "" {
	#         Ok(Err(Empty))
	#     } else {
	...
	# ## ```
	# input_text! : Element, Str => Try({}, [WebDriverError Str, ElementNotFound Str])
	...
	#     )
	#
	#     Host.element_send_keys!(session_id, element_id, str) |> Try.map_err(InternalError.handle_element_error)?
	#
	#     DebugMode.run_if_verbose!(
	#         |{}|
	...
	# ## input |> Element.clear!()?
	# ## ```
	# clear! : Internal.Element => Try({}, [WebDriverError Str, ElementNotFound Str])
	# clear! = |element|
	#     { session_id, element_id, selector_text, locator } = Internal.unpack_element_data(element)
	#
	#     DebugMode.run_if_verbose!(
	...
	#     )
	#
	#     Host.element_clear!(session_id, element_id) |> Try.map_err(InternalError.handle_element_error)?
	#
	#     DebugMode.run_if_verbose!(
	#         |{}|
	...
	# ## ```
	# find_element! : Element, Locator => Try(Element, [WebDriverError Str, ElementNotFound Str])
	...
	#     )
	#
	#     new_element_id = Host.element_find_element!(session_id, element_id, using, value) |> Try.map_err(InternalError.handle_element_error)?
	#
	#     DebugMode.run_if_verbose!(
	#         |{}|
	...
	# ## ```
	# try_find_element! : Element, Locator => Try([Found Element, NotFound], [WebDriverError Str, ElementNotFound Str])
	...
	# ## ```
	# find_single_element! : Element, Locator => Try(Element, [AssertionError Str, ElementNotFound Str, WebDriverError Str])
	...
	#             elements
	#             |> List.first
	#             |> Try.on_err(|_| crash("just checked - there is 1 element in the list"))
	#
	#         n ->
	#             (_, value) = Locator.get_locator(locator)
	...
	# ## ```
	# ##
	# find_elements! : Element, Locator => Try((List Element), [WebDriverError Str, ElementNotFound Str])
	...
	#     )
	#
	#     result = Host.element_find_elements!(session_id, parent_element_id, using, value) |> Try.map_err(InternalError.handle_element_error)
	#
	#     match result {
	#         Ok(element_ids) ->
	#             DebugMode.run_if_verbose!(
	...
	# ## ```
	# get_tag_name! : Element => Try(Str, [WebDriverError Str, ElementNotFound Str])
	...
	#     )
	#
	#     Host.element_get_tag!(session_id, element_id) |> Try.map_err(InternalError.handle_element_error)
	#
	...
	# ## ```
	# get_css_property! : Element, Str => Try(Str, [WebDriverError Str, ElementNotFound Str])
	...
	#     )
	#
	#     Host.element_get_css!(session_id, element_id, css_property) |> Try.map_err(InternalError.handle_element_error)
	#
	# ElementRect : {
	#     x : F64,
	#     y : F64,
	...
	# ## ```
	# get_rect! : Element => Try(ElementRect, [WebDriverError Str, ElementNotFound Str])
	# get_rect! = |element|
	#     { session_id, element_id, selector_text } = Internal.unpack_element_data(element)
	#
	#     DebugMode.run_if_verbose!(
	...
	#     )
	#
	#     Host.element_get_rect!(session_id, element_id)
	#     |> Try.map_ok(
	#         |list|
	#             match list {
	...
	#     )
	#     |> Try.map_err(InternalError.handle_element_error)
	#
	...
	# ## )
	# ## ```
	# use_iframe! : Element, (Internal.Browser => Try({}, _)) => Try({}, _)
	...
	#     )
	#
	#     Host.switch_to_frame_by_element_id!(session_id, element_id) |> Try.map_err(WebDriverError)?
	#
	#     DebugMode.run_if_debug_mode!(
	#         |{}|
	...
	#     )
	#
	#     Host.switch_to_parent_frame!(session_id) |> Try.map_err(WebDriverError)?
	#
	#     DebugMode.run_if_debug_mode!(
	#         |{}|
