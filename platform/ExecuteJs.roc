...
ExecuteJs :: [].{
	...
	# execute_js! : Browser, Str => Try(a, [WebDriverError Str, JsReturnTypeError Str]) where a implements Decoding
	# execute_js! = |browser, script| {
	#     { session_id } = Internal.unpack_browser_data(browser)
	#
	#     result_str = Host.execute_js!(session_id, script, "[]") |> Result.map_err(|err| WebDriverError(err))?
	...
	#
	#     result_str = Host.execute_js!(session_id, script, arguments_str) |> Result.map_err(WebDriverError)?
	#     result_utf8 = result_str |> Str.to_utf8
	#
	#     decoded : Result a _
	#     decoded = Decode.from_bytes(result_utf8, PropertyDecoder.utf8)
...
#
# expect
#     input = [String("wow"), Number(78.2), Number(0), Boolean(True), Boolean(False), Null]
